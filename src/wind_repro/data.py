...
def _penmanshiel_turbine_id(entry_name: str) -> str:
    match = re.search(
        r"Turbine_Data_Penmanshiel_(\d+)_",
        Path(entry_name).name,
        flags=re.IGNORECASE,
    )
    if not match:
        raise ValueError(
            f"Could not extract a turbine ID from Penmanshiel entry: {entry_name}"
        )
    return f"WT{int(match.group(1)):02d}"


def _penmanshiel_header_row(
    archive: zipfile.ZipFile, entry_name: str
) -> int:
    with archive.open(entry_name) as handle:
        for row_number, line in enumerate(handle):
            if line.lstrip().startswith(b"# Date and time,"):
                return row_number
            if row_number >= 50:
                break
    raise ValueError(
        f"Could not locate the Penmanshiel CSV header in {entry_name}"
    )


def _read_penmanshiel_entry(
    archive: zipfile.ZipFile,
    entry_name: str,
    data_config: dict[str, Any],
) -> pd.DataFrame:
    header_row = _penmanshiel_header_row(archive, entry_name)
    with archive.open(entry_name) as handle:
        header = pd.read_csv(handle, skiprows=header_row, nrows=0)

    timestamp_column = _find_column(
        header.columns, data_config.get("timestamp_column"), "timestamp"
    )
    speed_column = _find_column(
        header.columns, data_config.get("speed_column"), "speed"
    )
    direction_column = _find_column(
        header.columns, data_config.get("direction_column"), "direction"
    )
    usecols = [timestamp_column, speed_column, direction_column]
    with archive.open(entry_name) as handle:
        frame = pd.read_csv(
            handle,
            skiprows=header_row,
    ...
    return pd.DataFrame(
        {
            ...
            "turbine_id": _penmanshiel_turbine_id(entry_name),
            "wind_speed": pd.to_numeric(frame[speed_column], errors="coerce"),
            "wind_direction": pd.to_numeric(
                frame[direction_column], errors="coerce"
            ),
...
def _penmanshiel_archive_year(path: Path) -> int | None:
    match = re.search(r"SCADA_(\d{4})_", path.name, flags=re.IGNORECASE)
    return int(match.group(1)) if match else None


def _penmanshiel_static_path(
    input_path: Path, data_config: dict[str, Any]
) -> Path | None:
    configured = data_config.get("static_metadata_path")
    if configured:
        path = resolve_project_path(configured)
        if not path.exists():
            raise FileNotFoundError(
                f"Penmanshiel static metadata does not exist: {path}"
            )
        return path
    if input_path.is_dir():
        candidate = input_path / "Penmanshiel_WT_static.csv"
        if candidate.exists():
            return candidate
    return None


def _load_penmanshiel_static_coordinates(path: Path) -> pd.DataFrame:
    frame = pd.read_csv(path, low_memory=False)
    required = {"Alternative Title", "Latitude", "Longitude"}
    missing = required - set(frame.columns)
    if missing:
        missing_text = ", ".join(sorted(missing))
        raise ValueError(
            f"Penmanshiel static metadata is missing columns: {missing_text}"
        )

    turbine_number = frame["Alternative Title"].astype(str).str.extract(
        r"(?i)^T(\d+)$", expand=False
    )
    latitude = pd.to_numeric(frame["Latitude"], errors="coerce")
    longitude = pd.to_numeric(frame["Longitude"], errors="coerce")
    valid = turbine_number.notna() & latitude.notna() & longitude.notna()
    if not valid.any():
        raise ValueError(
            f"No valid turbine coordinates were found in {path}"
        )

    static = pd.DataFrame(
        {
            "turbine_id": turbine_number.loc[valid].map(
                lambda value: f"WT{int(value):02d}"
            ),
            "latitude": latitude.loc[valid],
            "longitude": longitude.loc[valid],
            "elevation": (
                pd.to_numeric(frame.loc[valid, "Elevation (m)"], errors="coerce")
                if "Elevation (m)" in frame.columns
                else np.nan
            ),
        }
    ).drop_duplicates("turbine_id", keep="last")

    earth_radius_m = 6_371_000.0
    latitude_radians = np.deg2rad(static["latitude"].to_numpy())
    longitude_radians = np.deg2rad(static["longitude"].to_numpy())
    origin_latitude = float(static["latitude"].mean())
    origin_longitude = float(static["longitude"].mean())
    origin_latitude_radians = np.deg2rad(origin_latitude)
    origin_longitude_radians = np.deg2rad(origin_longitude)
    static["x"] = (
        earth_radius_m
        * np.cos(origin_latitude_radians)
        * (longitude_radians - origin_longitude_radians)
    )
    static["y"] = earth_radius_m * (
        latitude_radians - origin_latitude_radians
    )
    static.attrs["coordinate_metadata"] = {
        "source": str(path),
        "geodetic_crs": "WGS84 latitude/longitude",
        "local_projection": "local equirectangular approximation",
        "units": "metres",
...
def _load_penmanshiel_archives(
    input_path: Path, data_config: dict[str, Any]
) -> pd.DataFrame:
    archives = (
        [input_path]
        if input_path.is_file()
        else sorted(input_path.glob("Penmanshiel_SCADA_*.zip"))
    )
    configured_years = data_config.get("years")
    years = {int(year) for year in configured_years} if configured_years else None
    if years is not None:
        archives = [
            path for path in archives if _penmanshiel_archive_year(path) in years
        ]
    if not archives:
        raise FileNotFoundError(
            f"No matching Penmanshiel ZIP archives were found under {input_path}"
        )

    frames: list[pd.DataFrame] = []
    for archive_path in archives:
        print(f"reading_archive={archive_path.name}")
        with zipfile.ZipFile(archive_path) as archive:
            ...
            for entry_name in entries:
                print(
                    f"reading_turbine={_penmanshiel_turbine_id(entry_name)} "
                    f"archive={archive_path.name}"
                )
                frames.append(
                    _read_penmanshiel_entry(archive, entry_name, data_config)
                )
    if not frames:
        raise FileNotFoundError(
            f"No turbine data CSV entries were found in {len(archives)} archives."
        )
    result = pd.concat(frames, ignore_index=True)
    static_path = _penmanshiel_static_path(input_path, data_config)
    if static_path is not None:
        static = _load_penmanshiel_static_coordinates(static_path)
        result = result.drop(columns=["x", "y"]).merge(
            static,
            on="turbine_id",
            how="left",
            validate="many_to_one",
        )
        missing_turbines = sorted(
            result.loc[
                result["latitude"].isna(), "turbine_id"
            ].drop_duplicates()
        )
        if missing_turbines:
            missing_text = ", ".join(missing_turbines)
            raise ValueError(
                "Penmanshiel static metadata has no coordinates for: "
                f"{missing_text}"
            )
        result.attrs["coordinate_metadata"] = static.attrs[
            "coordinate_metadata"
        ]
    return result
...
RAW_FORMAT_LOADERS: dict[
    ...
    "csv": _load_csv_files,
    "penmanshiel_zip": _load_penmanshiel_archives,
}
