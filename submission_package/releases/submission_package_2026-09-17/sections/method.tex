...
Consider a wind farm with $N$ turbines and a sampled sequence of length $T$. At time $t$, the wind speed and wind direction of turbine $i$ are denoted by $v_{t,i}$ and $\theta_{t,i}$. Given the past $L$ time steps, the model predicts the states of all turbines for the next $H$ time steps \cite{Box2015TimeSeriesAnalysis,Makridakis2020M4Competition}:
...
Raw SCADA records are aligned to a common 10-minute time axis. Wind-speed values outside $[0,25]\ \mathrm{m/s}$ and wind-direction values outside $[0,2\pi)$ are marked as missing. Missing speeds are linearly interpolated, while directions are interpolated after conversion to sine and cosine components \cite{Wang2024ContinuousMissingValuesImputationGAN}. For wind speed, low-amplitude frequency components are removed through FFT-based denoising before sliding-window generation; incomplete windows are discarded.
...
\label{sec:windformer_method}


\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\textwidth]{figures/WindFormer_architecture.png}
...
\label{fig:WindFormer_architecture}
\end{figure}

\subsection{Overall WindFormer Structure}

WindFormer is designed around two physical properties of wind farms: wind speeds tend to share common meteorological evolution across turbines, whereas wind directions are recorded as one-dimensional angles and exhibit stronger turbine-specific patterns. As shown in Fig.~\ref{fig:WindFormer_architecture}, WindFormer contains a wind-speed branch and a wind-direction branch. The branches are representation-decoupled so that each variable is modeled in its appropriate space, but they are jointly optimized through a physical wind-vector consistency objective.

Given an input window, the wind-speed branch decomposes the multi-turbine speed field into a common speed component and turbine-level residual components. The common component is encoded by Phase-Attention to capture phase-aligned evolution across temporal blocks, while the residual components are predicted by a shared residual MLP to recover local deviations. The predicted common speed and turbine residuals are then reconstructed into future turbine-wise wind speeds.
...
\subsection{Common Wind Speed Phase-Attention over Candidate Temporal Scales}

\textbf{Candidate temporal-scale folding.}
...
For a screened candidate temporal scale $p$, the model organizes the sequence into multiple local temporal blocks according to relative phase position. The scale is a modeling choice for organizing recurring local dynamics, rather than a claim that the wind farm follows one strict physical period \cite{Autoformer2021DecompositionAutoCorrelation,FEDformer2022FrequencyEnhanced}. Let:
...
Here, $K_p$ represents the number of temporal blocks contained in the historical window, and $p$ represents the number of phase positions within each block. Each block is flattened into a token:

\begin{equation}
\mathbf{q}^{(p)}_k = \operatorname{Flatten} \left( \mathbf{C}^{(p)}_{k,:,:} \right) \in \mathbb{R}^{p}.
\end{equation}

For multiple screened candidate scales collected in $\mathcal{P}$, the model organizes short-term temporal patterns at different local phase resolutions.
...
To aggregate context across different temporal blocks, the model introduces $R$ learnable router tokens. Let:

\begin{equation}
\mathbf{R}^{(p)} \in \mathbb{R}^{R\times d_m},
\end{equation}

then the routers first extract global information from the temporal-scale tokens:

\begin{equation}
\mathbf{C}^{(p)} = \operatorname{Attn} \left( \mathbf{R}^{(p)}, \mathbf{E}^{(p)}, \mathbf{E}^{(p)} \right),
\end{equation}

Subsequently, the temporal-scale tokens read cross-block context from the router representations:

\begin{equation}
\widetilde{\mathbf{E}}^{(p)} = \operatorname{Attn} \left( \mathbf{E}^{(p)}, \mathbf{C}^{(p)}, \mathbf{C}^{(p)} \right).
\end{equation}

With residual connections, layer normalization, and feed-forward networks, router tokens summarize cross-block context without full pairwise interaction among all temporal-scale tokens.

\textbf{Multi-scale common wind speed prediction.}

For each candidate temporal scale, the model concatenates the last block token with the average representation of all block tokens:

\begin{equation}
\mathbf{s}^{(p)} = \left[ \widetilde{\mathbf{e}}^{(p)}_{K_p}; \frac{1}{K_p} \sum_{k=1}^{K_p} \widetilde{\mathbf{e}}^{(p)}_k \right].
\end{equation}

Subsequently, the future common wind speed is obtained through the temporal-scale branch prediction head:

\begin{equation}
\hat{\mathbf{c}}^{(p)} = f_{\mathrm{head}}^{(p)} \left( \mathbf{s}^{(p)} \right) \in \mathbb{R}^{H}.
\end{equation}

The prediction results of different candidate temporal scales are combined by learnable fusion weights:

\begin{equation}
\hat{\mathbf{c}} = \sum_{p\in\mathcal{P}} \beta_p\hat{\mathbf{c}}^{(p)}, \qquad \beta_p = \frac{\exp(\gamma_p)}{\sum_{q\in\mathcal{P}}\exp(\gamma_q)}.
\end{equation}

Thus, Phase-Attention learns a fusion of multiple short-term candidate scales rather than relying on a single fixed temporal organization.
...
This patch representation captures local turning patterns and longer-range directional evolution \cite{Zerveas2021TimeSeriesRepresentation,Nie2024ChannelAwareLowRankAdaptation}.
